...
r43 returns [List res]
    : ids+=ID! (','! ids+=ID!)* {$res = map($ids, (id) => id.text);}
    ;
...
r46 returns [List res]
    : ids+=primary! (','! ids+=primary!)* {$res = map($ids, (id) => id.text);}
    ;
...
r50
    : ID 
        -> ^({new CommonTree.fromToken(new CommonToken(FLOAT, "1.0"))} ID)
    ;

/** templates tested:
    tokenLabelPropertyRef_tree
*/
r51 returns [Object res]
    : ID t=ID ID
        { $res = $t.tree; }
    ;
