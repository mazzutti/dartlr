...
class RecognizerSharedState {
  ...
  RecognizerSharedState() {
    this.following = new List<BitSet>(BaseRecognizer.INITIAL_FOLLOW_STACK_SIZE);
  }
  ...
  RecognizerSharedState.fromRecognizerSharedState(RecognizerSharedState state) {
    ...
    if (this.following.length < state.following.length)
        this.following = new List<BitSet>(state.following.length);
    Arrays.copy(state.following, 0, this.following, 0, state.following.length);
    this._fsp = state.fsp;
    ...
    if (state.ruleMemo != null)
        this.ruleMemo = new List<Map>.from(state.ruleMemo);
