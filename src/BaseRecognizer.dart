...
class BaseRecognizer {
  ...
  BaseRecognizer([this._state]) {
    if(this._state == null) 
      this._state = new RecognizerSharedState();
    this._reportedErros = new List<String>();
  }
  
  List<String> get reportedErrors() => this._reportedErros;
  
  String get output() => this._output;
  
  RecognizerSharedState get state() => this._state;
  ...
  Object matchSymbol(IntStream input, int ttype, BitSet follow) {
    Object matchedSymbol = this._getCurrentInputSymbol(input);
    if (input.LA(1) == ttype ) {
      input.consume();
      state.errorRecovery = false;
      state.failed = false;
    ...
    matchedSymbol = this._recoverFromMismatchedToken(input, ttype, follow); 
    return matchedSymbol;
  }

  /** Match the wildcard: in a symbol */
  void matchAny([IntStream input]) {
    this._state.errorRecovery = false;
  ...
  bool mismatchIsMissingToken(IntStream input, BitSet follow) {
    ...
    if (follow.member(Token.EOR_TOKEN_TYPE)) {
      BitSet viableTokensFollowingThisRule = this._computeContextSensitiveRuleFOLLOW();
  ...
  */
  void reportError(RecognitionException e) {    
    if (this._state.errorRecovery) return;
    this._state.syntaxErrors++;
    this._state.errorRecovery = true;
  ...
  void displayRecognitionError(List<String> tokenNames, RecognitionException e) {
    ...
    this.reportedErrors.add("$hdr $msg");
  ...
  String getErrorHeader(RecognitionException e) {
    if (this.sourceName != null)
      return "${this.sourceName} line ${e.line}:${e.charPositionInLine}";        
  ...
  void recover(RecognitionException re, [IntStream input]) {
    if (this._state.lastErrorIndex == input.index)
      input.consume();
    this._state.lastErrorIndex = input.index;
    BitSet followSet = this._computeErrorRecoverySet();
    this.beginResync();
    this.consumeUntilBitSet(input, followSet);
    this.endResync();
  }
   ...
   */
  BitSet _computeErrorRecoverySet() => this._combineFollows(false);
   ...
   */
  BitSet _computeContextSensitiveRuleFOLLOW() => this._combineFollows(true);
  ...
  BitSet _combineFollows(bool exact) {
    ...
    for (int i = top; i >= 0; i--) {
      BitSet localFollowSet = this._state.following[i];
  ...
  */
  Object _recoverFromMismatchedToken(IntStream input, int ttype, BitSet follow) {
    ...
    if (this.mismatchIsMissingToken(input, follow)) {     
      Object inserted = this._getMissingSymbol(input, e, ttype, follow);
  ...
  Object recoverFromMismatchedSet
    ...
    if (this.mismatchIsMissingToken(input, follow) ) {
      this.reportError(e);
      return this._getMissingSymbol(input, e, Token.INVALID_TOKEN_TYPE, follow);
    }
    throw e;
  }
  ...
  */
  ...
  void pushFollow(BitSet fset) {    
    if ((this._state.fsp + 1) >= this._state.following.length) {
      List<BitSet> f = new List<BitSet>(state.following.length*2);
      Arrays.copy(this._state.following, 0, f, 0, this._state.following.length);
      this._state.following = f;
    }
    this._state.following[++this._state.fsp] = fset;
   }
  ...
  */
  ...
  int getBacktrackingLevel() => this._state.backtracking;

  void setBacktrackingLevel(int n) { 
    this._state.backtracking = n; 
  }
  
  /** Return whether or not a backtracking attempt failed. */
  bool failed() => this._state.failed;
  ...
  abstract String get sourceName();
  
  abstract String getRecognizerClassName();
  ...
  int getRuleMemoization(int ruleIndex, int ruleStartIndex) {
    if (this._state.ruleMemo[ruleIndex] == null )
      this._state.ruleMemo[ruleIndex] = new HashMap();
    int stopIndexI = this._state.ruleMemo[ruleIndex][ruleStartIndex];
    if (stopIndexI == null)
  ...
  */
  void memoize(IntStream input, int ruleIndex, int ruleStartIndex) {
    int stopTokenIndex = this._state.failed ? MEMO_RULE_FAILED : input.index - 1;
    if (this._state.ruleMemo == null)
      print("!!!!!!!!! memo array is null for ${this.grammarFileName}");
    if (ruleIndex >= this._state.ruleMemo.length)
      print("!!!!!!!!! memo size is ${this._state.ruleMemo.length}, but rule index is $ruleIndex");
    if (this._state.ruleMemo[ruleIndex] != null)
  ...
  void traceIn(String ruleName, int ruleIndex, [Object inputSymbol])  {
    ...
    if (state.backtracking > 0) {
      print(" backtracking=${this._state.backtracking}");
    }
  ...
  void traceOut(String ruleName, int ruleIndex, [Object inputSymbol]) {
    ...
    if (this._state.backtracking > 0) {
      print(" backtracking=${this._state.backtracking}");
      if (this._state.failed) print(" failed");
